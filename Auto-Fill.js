(function () {
  if (window.LPAF && window.LPAF.installed) {
    alert("Auto-Fill already installed.\nRefresh the page to reinstall the updated version.");
    return;
  }
  ...
  function extractName(t, email, phone, postcode) {
    const lines = clean(t)
      .split(/\n+/)
      .map(x => x.replace(email, "").replace(phone, "").replace(postcode, "").trim())
      .filter(Boolean);

    const best = lines.find(l => {
      if (/[0-9@]/.test(l)) return false;
      const words = l.split(/\s+/).filter(Boolean);
      return words.length >= 2 && words.length <= 4 && words.every(w => /^[A-Za-z'’-]+$/.test(w));
    });

    if (!best) return { firstName: "", lastName: "" };
